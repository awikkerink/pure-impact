...
var DailyGetGraphData = function(days, todayLessXDays, apiRoute, split, toFixed, type, palette, cbvalues, dateLabels) {
  var dateLabels = []
  var getDate = function (dateLabelList) {
    apiGetRequest('/datekeys?days=' + (days + todayLessXDays - 1), function (res) {
      for (var i = 0; i < res.length - todayLessXDays; i++){
        var date = res[i].date
        var dateformatted = date.split(' ')[0].substring(0,3) + " " + date.split(' ')[1] + " " + date.split(' ')[2]
        dateLabels.push(dateformatted)
        if (res.length - todayLessXDays - 1 == i){
...
var PopulateGraphDaily = function(days, todayLessXDays, ticksAt0, lineThinkness, apiRoute, split, chartId, legendId, toFixed, type, stacked, palette, cb){
  DailyGetGraphData(days, todayLessXDays, apiRoute, split, toFixed, type, palette, function(cbvalues, dateLabels) {
    var scalesOptions = "{}"
    if (stacked == 'stacked'){
      stacked = true
    }
    ...
    var loginChartCanvas = $("#" + chartId).get(0).getContext("2d");
    ...
    window[legendId] = new Chart(loginChartCanvas, loginChartData);
